...
//@Autonomous(name = "Auto Crater 11")//lists this file as an Opmode to the phone as the name in quotes

public class Auto_Crater_TFec_11 extends LinearOpMode {
    ...
    */
    ...
    public void runOpMode() {
        ...
        //for recording X-axis location of 2 of 3 minerals

        vuforiaRoverRuckus = new VuforiaRoverRuckus();//start up our two objects
        tfodRoverRuckus = new TfodRoverRuckus();
        //boolean markerIsGold; once used
        ...
        //the initialize command was exported from the TFOD webcam code on the blocks programming interface
        //this was done early on, as we favored one large command compared to a bunch of small ones

        while (!opModeIsActive() && !isStopRequested()) {//make sure we can run our code in a loop
            telemetry.addData(">", "Press Play to start");
            telemetry.addData("Time: ", (int)this.time);
            telemetry.update();//this is keep alive telemetry to prevent temp disconnects-
        }                           //between phones when idling after init
        waitForStart();
        if (opModeIsActive()) {
            tfodRoverRuckus.activate();
            // Put run blocks here.
            gameTimeSnapShot = this.time;//take a snapshot, resetStartTime seemed faulty at times
            while (opModeIsActive() && this.time <= (2 + gameTimeSnapShot)) {//1 second,0.5, 1.5
                //allocate 2 seconds for looping through TFOD (tensorflow object recognition)
                if (tfodRoverRuckus != null) {
                    ...
                    if (updatedRecognitions != null) {
                        ...
                        if (updatedRecognitions.size() == 2) {//if it see's 2 minerals do all this...

                            goldMineralX = -1; silverMineral1X = -1; silverMineral2X = -1;
                            //reset old X-axis locations, as a value out of detectable range
                            telemetry.update();
                            ...
                            if ((goldMineralX != -1 && silverMineral1X != -1) || (silverMineral1X != -1  && silverMineral2X != -1)) {
                                //if it saw a gold and a silver, or if it saw 2 silvers...
                                if (goldMineralX < silverMineral1X && goldMineralX != -1) {
                                    //if it saw a gold that was to the left of the camera detection
                                    telemetry.addData("Gold Mineral Position", "Center");
                                    goldSample = 2;
                                } else if (goldMineralX > silverMineral1X) {
                                    //if it saw a gold that was to the right of the silver
                                    telemetry.addData("Gold Mineral Position", "Left");
                                    goldSample = 1;
                                } else {//also serves as a default upon misreading
            ...
            tfodRoverRuckus.deactivate();//close objects and dump memory, to save hardware usage
        ...
        *Now you see what the robot see's, what a headache!
        ...
        if (opModeIsActive() && this.time <= (27.5 + gameTimeSnapShot) && !isStopRequested()) {
            ...
            encoderDrive(7.3, 0,0,1, 0, 0,
                    7.5, 0, 0, false);//was lift 7
            //drop
            ...
            if (goldSample == 3) {  //1: left, 2: center, 3: right, -1: unknown
                ...
                sleep(2000);
                encoderDrive(7, 0.25, 0.25, 1, -70, -70, -4.0, 0, 0, false);
