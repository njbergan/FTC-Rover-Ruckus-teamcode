...
@Autonomous(name = "Auto Crater 14")//lists this file as an Opmode to the phone as the name in quotes

public class Auto_Crater_TFeci_14_NOTEBOOK extends LinearOpMode {
    ...
    *or Counts Per Inch. Then if on a wheel or gear that isn't 1 inch in diameter, a multiplier is included
    */
    ...
    public void runOpMode() {
        ...
        //so you don't have to wire red to black, to maintain program logic (now illegal to crosswire)
        ...
        // Initialize our camera objects
        vuforiaRoverRuckus.initialize("", hardwareMap.get(WebcamName.class, "Webcam1"),
                "teamwebcamcalibrationLive.xml", false,
                false, VuforiaLocalizer.Parameters.CameraMonitorFeedback.AXES,
                0, 0, 0, 0, 0, 0, true);
        ...
        if (!opModeIsActive() && !isStopRequested()) {
            tfodRoverRuckus.activate();//start scanning for minerals
            gameTimeSnapShot = this.time;
            while (!opModeIsActive() && !isStopRequested()) {
                if (tfodRoverRuckus != null) {
                    // If a RoverRuckus element is able to be, and is detected, put it in the list
                    List<Recognition> updatedRecognitions = tfodRoverRuckus.getUpdatedRecognitions();
                    if (updatedRecognitions != null) {
                        //If there are recognized elements, display a count of elements
                        telemetry.addData("# Object Detected", updatedRecognitions.size());

                        //Must be on 2 because webcam is designed to only see 2
                        if (updatedRecognitions.size() == 2) {  //when webcam see's 2 minerals:

                            //(re)set all three minerals to an impossible, placeholder X(horizontal) value
                            goldMineralX = -1;
                            silverMineral1X = -1;
                            silverMineral2X = -1;
                            telemetry.update();
                            ...
                            for (Recognition recognition : updatedRecognitions) {
                                //if it detects gold, set it's X, if it detects silver for the first time, assign it to 'OneX'
                            ...
                            if ((goldMineralX != -1 && silverMineral1X != -1) || (silverMineral1X != -1 && silverMineral2X != -1)) {
                                //if gold and silver, or silver and silver, are found...
                                //find the location of the two minerals found, relative to eachother's X(horizontal) position
            ...
            }   //continue the while loop until this line (!opmodeIsActive && !stopRequested)
            //once play has been pressed, close all camera objects to save resources.
            waitForStart();
        ...
        *Now you see what the robot see's, what a headache!
        ...
        gameTimeSnapShot = this.time;//take a new snapshot (record the current game time)
            //we've had some problems with the resetStartTime command

        //logical robot movements based on encoders

        if (opModeIsActive() && this.time <= (27.5 + gameTimeSnapShot) && !isStopRequested()) {
        //limits code to 30 seconds with spare time to compensate for a slow phone and any unclosed processes
    ...
    * This is because you can't turn if you don't have each motor's distance direction controlled, see axillary power
    * speed, leftInches, rightInches, motorLiftInches
    */

            //All encoderDrive functions move the robot in terms of the robot's perspective
            ...
            //strafe off hook, to the robot's left

            encoderDrive(2, 0, 0, 1, 0, 0,
                    -4, 0, 0, false);
            //contract actuator to halfway

            sleep(100);
            //encoderDrive(0.5, 0.4,0.4,0, -3, -3, 0, 0, 0, false);
            //backup to straighten
                    //now not needed, after adding counterweight to front of robot


//Diverging paths
            if (goldSample == 3) {  //Gold Positions 1: left, 2: center, 3: right, -1: unknown
                encoderDrive(0.85, 0.52,0.15,0, 180, 60,
                        0, 0, 0, false);//0.15,0.45,15",45"
                    //Forward arc to the right  s0.9  L52
                sleep(100);
                encoderDrive(0.55, 0.52,0.15,0, -180, -60,
                        0, 0, 0, false);
                //Partially undo the arc    0.15,0.45,15",45"     //s0.5
                sleep(250);
                encoderDrive(2, 0.35,0.35,0, -18.5, 18.5,
                        0, 0, 0, false);
                //Tank turn to the left     18    //L-16.75,19,20,19
                //encoderDrive(2.4, 0.28, 0.40, 0, 200, 240, 0, 0, 0, false);
                    //R37,47,37,39,35. L25  s2.5
                sleep(250);
                encoderDrive(1.9, 0.25,0.4,0, 60, 180,
                        0, 0, 0, false);
                //Make a long arc to the depot      0.15,0.45,15",45"
                sleep(100);     //timeout 1.75,1.85     L25     R40
                encoderDrive(2, 0.4, 0.4, 0, 18, 18,
                        0, 0, 0, false);
                                //Straight drive into the depot     TO2       "12
                motorFlapper.setPower(-0.5);//half power
                sleep(2000);
                //encoderDrive(5, 0.25,0.25,1, -68, -68, -4.0, 0, 0, false);
                encoderDrive(5, 0.25,0.25,1, -68, -68,
                        0, 0, 0, false);
                //backup into the crater

                //timeout 7, now 5

            } else if (goldSample == 2) {
                encoderDrive(2, 0.3, 0.3, 0, 21, 21,
                        0, 0, 0, false);//25"
                //drive forward to crater          23.5
                encoderDrive(1.2, 0.3, 0.3, 0, -10, -10,
                        0, 0, 0, false);
                //backup from minerals
                encoderDrive(2, 0.35, 0.35, 0, -15, 15,
                        0, 0, 0, false);//18
                //tankturn left
                encoderDrive(2, 0.27, 0.41, 0, 200, 240,
                        0, 0, 0, false);
                //arc drive to the left      LdtS 25, RdtS 36, Seconds2.5,2.3
                motorFlapper.setPower(-0.5);
                sleep(2000);
                //encoderDrive(5, 0.25, 0.25, 1, -70, -70, -4.0, 0, 0, false);
                encoderDrive(5, 0.25, 0.25, 1, -63, -63,
                        0, 0, 0, false);
                //back drive to crater          -70,-68,-65     //mtrLeft 68, mtrRight 68
            } else {
            //} else if (goldSample == 1) {
                encoderDrive(0.8, 0.15,0.52,0, 60, 180,
                        0, 0, 0, false);
                //arc drive to the left     0.15,0.45,15",45"     //s0.9, 0.85  L52
                sleep(100);
                encoderDrive(0.5, 0.15,0.52,0, -60, -180,
                        0, 0, 0, false);
                //Partially undo the arc    0.15,0.45,15",45"   //s0.5, 0.55
                sleep(250);
                encoderDrive(2, 0.35,0.35,0, -10.5, 10.5,
                        0, 0, 0, false);//18
                //tank turn to left      R11
                //encoderDrive(2.4, 0.28, 0.40, 0, 200, 240, 0, 0, 0, false);
                //R37,47,37,39,35. L25  s2.5
                sleep(250);
                encoderDrive(1.9, 0.25,0.41,0, 60, 180,
                        0, 0, 0, false);
                //arc drive to the left     0.15,0.45,15",45"
                sleep(100);     //timeout 1.75,1.85     L25     R40
                encoderDrive(2, 0.4, 0.4, 0, 12, 12,
                        0, 0, 0, false);
                //Drive into depot      TimeOut 2  "12
                motorFlapper.setPower(-0.5);
                sleep(2000);
                //encoderDrive(5, 0.25,0.25,1, -68, -68, -4.0, 0, 0, false);
                encoderDrive(5, 0.25,0.25,1, -68, -68,
                        0, 0, 0, false);
                //Back drive into crater        timeout 7, now 5

            }

            //Lift-Go-Back-Down
            //encoderDrive(5, 0,0,1, 0, 0, -3.5, 0, 0, false);

            //turn off motorFlapper once paths are complete
    ...
    //lDriveTrains, rDTs, L", R" may be attainable with [fastestDTs, longestDT", ratio(positive and/or negative)]

    //mtrLiftInches, mtrShoulderDeg, and mtrRailInches are all not used ever, but added to be proactive

    /**
     * function to control all motors of the robot using their encoders
     *
    ...
    public void encoderDrive(double timeOut, double leftDTSpeed, double rightDTSpeed, double auxSpeed, double mtrLeftInches, double mtrRightInches,
        ...
        if (opModeIsActive()) {
            ...
            while (opModeIsActive() && (motorLeft.isBusy() || motorRight.isBusy() ||
                ...
                //If the time limit duration has passed, stop the repeat at the 'break'
    ...
    //A simple variable distributor assignment method,
    ...
    //another assignment method distributing a single call to all motors
    ...
    //Since target position is direction dependent, a strafe will need to change the direction of-
    //the front drive motors from the back.
